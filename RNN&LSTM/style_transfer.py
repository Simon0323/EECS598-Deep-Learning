...
def content_loss(content_weight, content_current, content_target):
    ...
    - scalar content loss
    """ 
    return torch.sum((content_current - content_target)**2)*content_weight
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        ...
        loss += torch.sum((gram_current-style_targets[i])**2*style_weights[i])
    return loss
...
def style_transfer(content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    for t in range(200):
        ...
        content_target = content_target.type(dtype)
        loss = content_loss(content_weight, feats[content_layer], content_target).type(dtype)
        loss += style_loss(feats, style_layers, style_targets,style_weights).type(dtype) 
        loss += tv_loss(img, tv_weight).type(dtype)
        loss.backward()
