...
def finetune(device, dataloaders, dataset_sizes, class_names):
    ...
    ####################################################################################
    din = model_ft.fc.in_features
    model_ft.fc = nn.Linear(din,2)
    ...
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=2, gamma=0.6)
...
def freeze(device, dataloaders, dataset_sizes, class_names):
    ...
    din = model_conv.fc.in_features
    model_conv.fc = nn.Linear(din,2)
    ...
    ####################################################################################
    criterion = nn.CrossEntropyLoss()
    optimizer_conv = optim.Adam(model_conv.parameters(), lr = 2e-5)
    ...
    # Decay LR by a factor of 0.1 every 7 epochs
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer_conv, step_size=2, gamma=0.6)
